...
public class ConfigLoaderV2 implements Loader {
	...
	private List<ServerConfig> loadServers(List<Variable> variables, JsonObject jsonConfig) {
		List<ServerConfig> ret = new ArrayList<>();
		JsonArray servers = jsonConfig.getJsonArray("servers");
		for (JsonValue v : servers) {
			JsonObject config = v.getJsonObject();
	...
	private boolean compatibleVersion(JsonObject config) {
		...
		if (!config.hasKey("version") || config.getInt("version") != 2) {
			return false;
		}
